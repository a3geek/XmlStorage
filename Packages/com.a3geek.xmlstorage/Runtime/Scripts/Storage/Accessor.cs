...
namespace XmlStorage
{
    public static partial class Storage
    {
        ...
        {
            var group = CurrentDataGroup;
            return group.TryGet(type, out var dictionary)
                ? dictionary.Keys
                : Array.Empty<string>();
        }
        ...
        {
            var group = CurrentDataGroup;
            return group.TryGet(type, out var dictionary) && dictionary.ContainsKey(key);
        }

        public static bool Delete(in Type type, in string key)
        {
            var group = CurrentDataGroup;
            return group.TryGet(type, out var dictionary) && dictionary.Remove(key);
        }
        ...
        public static int GetInt(in string key, int defaultValue = default)
        {
            return GetValue(key, defaultValue);
        }

        public static float GetFloat(in string key, float defaultValue = default)
        {
            return GetValue(key, defaultValue);
        }

        public static bool GetBool(in string key, bool defaultValue = default)
        {
            return GetValue(key, defaultValue);
        }
